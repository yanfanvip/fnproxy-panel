...
FnProxy Panel 是一个基于 Go 的轻量级服务管理面板，用于统一管理网站管理、反向代理、静态站点、跳转规则、证书、OAuth 访问控制、用户、SSH 终端和运行状态。该项目采用现代化的部署方式，支持多种操作系统和部署场景。
...
FnProxy Panel 提供了完整且灵活的部署方案，支持多种部署场景和平台。其设计特点包括：
