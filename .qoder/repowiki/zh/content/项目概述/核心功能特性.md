...
- [firewall_handler.go](file://src/handlers/firewall.go)
...
## 更新摘要
**变更内容**
- 新增防火墙功能模块，提供基于IP和国家的访问控制
- 更新网站管理功能描述，强调从端口监听到网站管理的演进
- 完善安全防护体系，增加防火墙中间件的架构位置
- 新增防火墙配置管理和规则管理的API接口说明
...
## 简介
本文件面向 FnProxy Panel 项目，系统性梳理其九大核心功能特性：网站管理、服务规则管理、HTTPS 证书管理、动态证书选择、OAuth 访问控制、用户管理、Token 鉴权、SSH/终端管理、运行监控、防火墙安全防护。每个功能均给出简要说明、典型使用场景与对应的实现要点，帮助读者快速理解与上手。

**更新** 新增防火墙安全防护功能，提供基于IP和国家的访问控制能力，完善安全防护体系。

## 项目结构
项目采用 Go Module 结构，入口为 main.go，API 路由在主函数中集中注册，业务处理集中在 handlers 包，模型定义在 models 包，工具类与监控在 utils 包，中间件在 middleware 包，安全与审计在 security 包，前端静态资源内嵌在 static 目录。新增防火墙功能通过独立的 handlers 和 middleware 模块实现。

```mermaid
graph TB
A["入口 main.go<br/>启动 HTTP 服务器与代理服务器"] --> B["路由注册<br/>/api/* 与 /ws/*"]
B --> C["防火墙中间件<br/>FirewallMiddleware"]
C --> D["认证中间件<br/>AuthMiddleware/CORS/Logging"]
D --> E["处理器 handlers/*<br/>API 业务逻辑"]
E --> F["模型 models/*<br/>数据结构定义"]
E --> G["工具 utils/*<br/>证书/监控/系统工具"]
E --> H["中间件 middleware/*<br/>认证/跨域/日志"]
E --> I["安全 security/*<br/>审计/密码/密钥"]
E --> J["静态资源 static/*<br/>内嵌前端资源"]
```

**图表来源**
- [main.go:421-427](file://src/main.go#L421-L427)
- [firewall_middleware.go:13-56](file://src/middleware/firewall.go#L13-L56)
...
## 核心组件
- **网站管理**：监听器（端口/协议/启停/热重载/状态），与代理服务器集成，支持端口占用检测与启动失败回退。**演进** 从简单的端口监听发展为完整的网站管理平台。
...
- **防火墙安全防护**：基于IP和国家的访问控制，支持规则优先级排序和默认拒绝策略。

**更新** 新增防火墙安全防护组件，提供多层次的安全访问控制能力。

**章节来源**
- [README.md:7-18](file://README.md#L7-L18)

## 架构总览
下图展示从客户端请求到业务处理与代理转发的整体流程，以及关键组件之间的交互，包括新增的防火墙中间件。
...
participant Auth as "认证/跨域/日志中间件"
...
**更新** 网站管理已从简单的端口监听发展为完整的网站管理平台，支持更丰富的配置和管理功能。
...
- **典型使用场景**
  ...
  - 实现区域性的访问控制策略。
- **关键实现**
  - 防火墙中间件：[firewall_middleware.go:13-56](file://src/middleware/firewall.go#L13-L56)
  - 防火墙处理器：[firewall_handler.go:21-68](file://src/handlers/firewall.go#L21-L68)
  - 配置管理：[config_manager.go:644-682](file://src/config/manager.go#L644-L682)
  - 规则匹配算法：[firewall_middleware.go:153-240](file://src/middleware/firewall.go#L153-L240)

```mermaid
flowchart TD
Request["客户端请求"] --> FW["防火墙中间件"]
FW --> CheckEnabled{"防火墙启用？"}
CheckEnabled --> |否| Allow["放行请求"]
CheckEnabled --> |是| CheckLocal{"localhost？"}
CheckLocal --> |是| Allow
CheckLocal --> |否| GetIP["获取客户端IP"]
GetIP --> MatchRules["匹配规则"]
MatchRules --> Action{"匹配结果"}
Action --> |允许| Allow
Action --> |拒绝| Deny["返回403 Forbidden"]
Action --> |无匹配| Default{"默认拒绝？"}
Default --> |是| Deny
Default --> |否| Allow
```
...
**图表来源**
- [firewall_middleware.go:13-56](file://src/middleware/firewall.go#L13-L56)
- [firewall_middleware.go:153-240](file://src/middleware/firewall.go#L153-L240)

**章节来源**
- [main.go:386-413](file://src/main.go#L386-L413)
- [firewall_handler.go:21-201](file://src/handlers/firewall.go#L21-L201)
- [firewall_middleware.go:13-241](file://src/middleware/firewall.go#L13-L241)
- [config_manager.go:644-739](file://src/config/manager.go#L644-L739)
...
Middleware --> Utils
Utils --> Models
Firewall["firewall_middleware"] --> Auth["auth_middleware"]
Auth --> Handlers
```
...
- **防火墙性能**
  - **新增** 规则匹配采用排序算法，按优先级遍历，避免不必要的匹配。
  - IP段匹配使用CIDR格式，支持高效网络地址计算。
  - 本地回环地址直接放行，减少规则匹配开销。
- **建议**
  - 大规模并发场景下，适当增大日志保留条数与网络采样频率的平衡。
  - 证书同步周期可根据外部证书变更频率调整。
  - **新增** 防火墙规则数量较多时，合理设置规则优先级，避免过多的规则匹配开销。
...
- **防火墙访问被拒绝**
  - **新增** 现象：访问被防火墙拒绝，返回403 Forbidden。
  - 处理：检查防火墙配置，确认客户端IP是否在允许列表中，检查规则优先级设置。
  - 参考：[firewall_middleware.go:13-56](file://src/middleware/firewall.go#L13-L56)，[firewall_handler.go:21-68](file://src/handlers/firewall.go#L21-L68)
...
- [auth_middleware.go:23-54](file://src/middleware/auth.go#L23-L54)
- [firewall_middleware.go:13-56](file://src/middleware/firewall.go#L13-L56)
- [firewall_handler.go:21-68](file://src/handlers/firewall.go#L21-L68)

## 结论
FnProxy Panel 项目围绕"统一管理"目标，提供了从监听器启停、服务规则配置、证书管理、OAuth 访问控制、用户与 Token 鉴权、SSH/终端管理到运行监控的完整能力闭环。**更新** 新增的防火墙安全防护功能进一步完善了安全体系，提供基于IP和国家的访问控制能力。

通过清晰的模块划分与中间件体系，既保证了易用性，也为扩展与维护提供了良好基础。防火墙中间件位于认证中间件之前，形成多层次的安全防护。建议在生产环境中合理配置安全参数、证书策略与监控阈值，同时充分利用防火墙功能实现更精细的访问控制，以获得更稳定与可观测的运行体验。

**新增** 随着防火墙功能的加入，系统从单一的网站管理平台发展为具备完善安全防护能力的企业级管理面板，能够更好地满足生产环境的安全需求。